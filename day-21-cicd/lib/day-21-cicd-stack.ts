...
// ════════════════════════════════════════════════════════════════
// Day 20 = Day 19 위에 "관측성(observability)" 한 묶음을 얹는다. 비즈니스 로직은 불변.
//   1) X-Ray 분산추적: workerFn/apiFn/discordFn 에 Tracing.ACTIVE + SDK 클라이언트 X-Ray 래핑(코드).
//      → 한 trace 에서 API → Worker → Bedrock/DDB/CostExplorer 흐름이 보인다.
//      (Lambda@Edge 는 X-Ray active tracing 미지원 → edgeFn 은 제외.)
//   2) CloudWatch 대시보드: 세 람다의 호출/에러/지연 + Bedrock 지표를 한 화면에.
//   3) 알람 + SNS: Worker/API 에러가 나면 SNS 토픽으로 통보(이메일은 `-c alertEmail=` 로 구독).
//   ※ "월 비용 예산"은 Day 17 awsCost 에서 비용을 다뤘으므로 이 day 에선 제외(운영 지표에 집중).
//
// Day 21 = 이 에이전트 스택 자체는 그대로. 바깥에 CI/CD(GitHub Actions OIDC)를 붙여 "이 스택을
//   키 없이 자동 배포"한다. OIDC 배포 역할은 별도 pipeline-stack 에, 워크플로는 repo 루트 .github/ 에.
// ════════════════════════════════════════════════════════════════

export class Day21CicdStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Day 20: 알람을 받을 이메일(옵션). `-c alertEmail=you@example.com` 주면 SNS 구독을 만든다.
    ...
    const workerFn = new nodejs.NodejsFunction(this, 'WorkerFunction', {
      ...
      tracing: lambda.Tracing.ACTIVE, // Day 20: X-Ray (xray:Put* 권한은 CDK 가 자동 부여)
      bundling: {
        ...
        externalModules: [
          ...
          // Day 20: aws-xray-sdk-core 안의 v2(aws-sdk) capture 경로는 안 쓰지만 esbuild 가 스캔하므로 external 로.
          'aws-sdk',
        ],
      },
      environment: {
    ...
    const apiFn = new nodejs.NodejsFunction(this, 'ApiFunction', {
      ...
      tracing: lambda.Tracing.ACTIVE, // Day 20: X-Ray. lambdaClient 래핑과 함께 API→Worker 가 이어진다.
      bundling: {
        ...
        externalModules: [
          ...
          '@aws-sdk/client-iot',
          '@aws-sdk/client-sts',
          'aws-sdk', // Day 20: aws-xray-sdk-core 의 v2 경로 미사용 — esbuild 회피용 external.
        ],
      },
      environment: {
    ...
    const discordFn = new nodejs.NodejsFunction(this, 'DiscordFunction', {
      ...
      tracing: lambda.Tracing.ACTIVE, // Day 20: 함수 단위 X-Ray 세그먼트(클라이언트 래핑은 안 함 — 간소화).
      bundling: {
        format: nodejs.OutputFormat.ESM,
        target: 'node20',
    ...
    // ════════════════════════════════════════════════════════════════
    //  Day 20: CloudWatch 대시보드 + 알람 + SNS
    // ════════════════════════════════════════════════════════════════
    const MODEL_ID = 'global.anthropic.claude-haiku-4-5-20251001-v1:0';

    // 알람을 흘릴 SNS 토픽. alertEmail context 가 있으면 이메일 구독까지(확인 메일 1회 클릭 필요).
    const alertTopic = new sns.Topic(this, 'AlertTopic', { displayName: 'serverless-agent alerts' });
    ...
    // 람다별 핵심 지표 헬퍼.
    const errors = (fn: lambda.IFunction) => fn.metricErrors({ period: cdk.Duration.minutes(5), statistic: 'Sum' });
    const invocations = (fn: lambda.IFunction) => fn.metricInvocations({ period: cdk.Duration.minutes(5), statistic: 'Sum' });
    const durationP99 = (fn: lambda.IFunction) => fn.metricDuration({ period: cdk.Duration.minutes(5), statistic: 'p99' });

    // 알람: Worker / API 에서 5분 내 에러가 1건이라도 나면 통보.
    errors(workerFn).createAlarm(this, 'WorkerErrorsAlarm', {
      threshold: 1, evaluationPeriods: 1, alarmDescription: 'Worker(Agent Loop) errors >= 1 / 5min',
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
    }).addAlarmAction(alarmAction);
    errors(apiFn).createAlarm(this, 'ApiErrorsAlarm', {
      threshold: 1, evaluationPeriods: 1, alarmDescription: 'API errors >= 1 / 5min',
      comparisonOperator: cloudwatch.ComparisonOperator.GREATER_THAN_OR_EQUAL_TO_THRESHOLD,
      treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
    }).addAlarmAction(alarmAction);

    // Bedrock 모델 호출 지연(있을 때만) — AWS/Bedrock 네임스페이스.
    const bedrockInvocations = new cloudwatch.Metric({
      namespace: 'AWS/Bedrock', metricName: 'Invocations',
      dimensionsMap: { ModelId: MODEL_ID }, period: cdk.Duration.minutes(5), statistic: 'Sum',
    });
    const bedrockLatency = new cloudwatch.Metric({
      namespace: 'AWS/Bedrock', metricName: 'InvocationLatency',
      dimensionsMap: { ModelId: MODEL_ID }, period: cdk.Duration.minutes(5), statistic: 'Average',
    });

    // 대시보드: 한 화면에서 호출/에러/지연 + Bedrock.
    const dashboard = new cloudwatch.Dashboard(this, 'Dashboard', {
      dashboardName: 'serverless-agent',
    });
    dashboard.addWidgets(
      new cloudwatch.GraphWidget({
        title: 'Invocations (5m)', width: 12,
        left: [invocations(apiFn), invocations(workerFn), invocations(discordFn)],
      }),
      new cloudwatch.GraphWidget({
        title: 'Errors (5m)', width: 12,
        left: [errors(apiFn), errors(workerFn), errors(discordFn)],
      }),
    );
    dashboard.addWidgets(
      new cloudwatch.GraphWidget({
        title: 'Duration p99 (ms)', width: 12,
        left: [durationP99(apiFn), durationP99(workerFn), durationP99(discordFn)],
      }),
      new cloudwatch.GraphWidget({
        title: 'Bedrock — Claude Haiku', width: 12,
        left: [bedrockInvocations], right: [bedrockLatency],
      }),
    );

    // ── 11) Outputs ──
    new cdk.CfnOutput(this, 'DashboardUrl', {
      value: `https://console.aws.amazon.com/cloudwatch/home?region=${this.region}#dashboards/dashboard/serverless-agent`,
      description: 'CloudWatch 대시보드',
    });
    new cdk.CfnOutput(this, 'XRayConsoleUrl', {
      value: `https://console.aws.amazon.com/cloudwatch/home?region=${this.region}#xray:traces/query`,
      description: 'X-Ray trace map / traces',
    });
