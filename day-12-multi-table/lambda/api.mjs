// Day 12 API handler — Hono + Lambda Function URL (BUFFERED).
//
// Day 11 과의 차이 (핵심): 단일 ConversationsTable → 도메인별 3테이블.
//   USERS_TABLE     PK id
//   SESSIONS_TABLE  PK user_id, SK id          (원본 chat-sessions)
//   MESSAGES_TABLE  PK session_id, SK created_at_id (원본 chat-messages)
//
// 분리가 새로 열어준 접근 패턴:
//   GET /users/:userId/sessions = SessionsTable.Query(user_id) 한 번.
//   Day 11 단일 테이블에선 sessionId 를 모르면 Scan 외엔 불가능했던 질의.
//
// 라우트 계층:
//   POST /users                      → 유저 생성
//   POST /users/:userId/sessions     → 세션 생성(유저 소속)
//   GET  /users/:userId/sessions     → 유저의 세션 목록 (★ 분리로 열린 패턴)
//   POST /chat                       → { userId, sessionId, message } → user msg Put + Worker invoke
//   GET  /sessions/:sessionId/messages → 세션 메시지 시간순 (Day 11 로직, 키 이름만 변경)
//
// Day 11 책임 분리 유지: API 는 Bedrock 권한 없음. Worker 만 모델을 부른다.
...
app.get("/health", (c) => c.json({ ok: true, day: 12, role: "api" }));
