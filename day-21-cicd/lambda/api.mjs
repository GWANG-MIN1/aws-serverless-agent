...
// Day 20: SDK 클라이언트를 X-Ray 로 감싸 trace 의 subsegment 로 잡는다.
//   특히 lambdaClient(Worker invoke)가 잡혀야 API→Worker 가 한 trace 로 이어진다.
...
const iot = AWSXRay.captureAWSv3Client(new IoTClient({}));
...
app.get("/health", (c) => c.json({ ok: true, day: 21, role: "api" }));
