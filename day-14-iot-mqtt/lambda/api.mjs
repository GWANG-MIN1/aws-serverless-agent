// Day 14 API handler — Hono + Lambda Function URL (BUFFERED).
//
// Day 13 과 동일하다. Day 14 의 변화는 전부 Worker 쪽(단계별 MQTT publish)이고,
// API 가 하는 일은 그대로다: user msg Put + Worker async invoke + 조회.
//   (API 는 Bedrock 도 IoT 도 모른다 — 실시간 출력은 Worker 의 책임.)
//   USERS_TABLE     PK id
...
app.get("/health", (c) => c.json({ ok: true, day: 14, role: "api" }));
