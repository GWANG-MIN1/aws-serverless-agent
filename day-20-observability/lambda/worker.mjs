// Day 20 Worker handler — 기존 Agent Loop + skills + Discord 위에 ★ X-Ray 계측(관측성).
//   Day 20 변화: AWS SDK 클라이언트(Bedrock/DDB/CE)를 AWSXRay.captureAWSv3Client 로 감싼다.
//   Lambda 의 Tracing.ACTIVE 와 합쳐져, 한 trace 에서 API→Worker→Bedrock 흐름이 보인다. 로직은 불변.
//
// 입력 (event): { type: "run_chat", userId, sessionId, message, createdAtId,
//                 channel?, interactionToken?, applicationId? }  ← Day 18: discord 채널 표식
//
// Day 18 한 가지: 루프가 끝나면 event.channel==="discord" 일 때 그 interaction 의 followup webhook 을
//   PATCH 해서 "생각 중…"(deferred)을 최종 텍스트로 바꾼다. Agent Loop/skill/MQTT 골격은 전부 그대로.
//
...
import { listPublicCalendarEvents } from "./calendar.mjs";

// Day 20: AWS SDK 클라이언트를 X-Ray 로 감싸면 각 호출이 trace 의 subsegment 로 잡힌다.
//   → Lambda 의 Tracing.ACTIVE 와 함께, 한 trace 에서 Worker → Bedrock / DynamoDB / Cost Explorer 가 보인다.
//   (active tracing 컨텍스트가 없으면 captureAWSv3Client 는 그냥 통과 — 로컬에서도 안전.)
...
const ce = AWSXRay.captureAWSv3Client(new CostExplorerClient({ region: "us-east-1" }));
...
// Day 19: iCloud 공개 ICS fetch/parse 시간을 포함한다.
...
const CALENDAR_TIME_ZONE = process.env.CALENDAR_TIME_ZONE ?? "Asia/Seoul";

const buildSystemPrompt = () => [
  "You are Claude, an AI assistant created by Anthropic.",
  "When asked about yourself, identify as Claude made by Anthropic.",
  `Current time: ${new Date().toISOString()}. Calendar timezone: ${CALENDAR_TIME_ZONE}.`,
  "",
  ...
  "",
  "  await calendar({ range?, from?, to?, limit? })",
  "    → reads events from the configured iCloud public calendar (read-only).",
  "    range: 'today' | 'this_week' | 'next_7_days' (default next_7_days).",
  "    Or pass from/to together as ISO date-time strings. Never invent calendar events.",
  "    Use fromLocalDate/toLocalDateExclusive when explaining the queried date range;",
  "    the raw from/to values are UTC instants and can show the previous local date.",
  "    For every event, COPY answerLine exactly and keep the returned event order.",
  "    Never regroup events by date, calculate a weekday, rewrite a time, or move an",
  "    event under another date. Do not use raw start/end/startLocal/endLocal.",
  "    Do not add '(일정 없음)' dates that were not returned by the skill.",
  "    If warning says the public feed has no VEVENT entries, do NOT say merely that the",
  "    requested range is empty. Explain that the named public calendar itself currently",
  "    publishes zero events and ask the user to check each event's Calendar field.",
  "  For ANY question about the user's schedule/calendar, call calendar(...) and read() the result.",
  "  This skill cannot create, edit, or delete events. Never offer to add or modify an event.",
  "",
...
const EXECUTE_CODE_TOOL = {
  toolSpec: {
    ...
    description:
      ...
      "You only receive what you read(). Safe globals (JSON/Date/Math/...) plus skills: " +
      "`await awsCost({start?,end?,granularity?,metric?,groupByService?})` returns the account's real AWS cost. " +
      "`await calendar({range?,from?,to?,limit?})` returns read-only events from the configured iCloud calendar. " +
...
async function runSandbox(code) {
  ...
  async function calendar(args = {}) {
    ...
    try {
      ...
      skillCalls.push({ name: "calendar", input: args, ok: true, ms: Date.now() - startedAt });
      return result;
    } catch (e) {
      skillCalls.push({ name: "calendar", input: args, ok: false, error: e?.message ?? String(e) });
      throw e;
    }
  }
  ...
  const sandbox = {
    ...
    calendar, // ★ Day 19: 설정된 iCloud 공개 ICS만 읽는 skill
    console,
    JSON, Date, Math,
...
export const handler = async (event) => {
  ...
  // 4) Day 18: Discord 채널이면 deferred 응답을 최종 텍스트로 채운다(followup webhook PATCH).
  //    interaction token 이 인증을 대신하므로 bot token 불필요. 글로벌 fetch(Node20). best-effort.
  if (event.channel === "discord" && event.interactionToken && event.applicationId) {
    try {
      const url = `https://discord.com/api/v10/webhooks/${event.applicationId}/${event.interactionToken}/messages/@original`;
      const res = await fetch(url, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ content: (finalText || "(빈 응답)").slice(0, 2000) }), // Discord 메시지 2000자 한도.
      });
      if (!res.ok) console.warn("Worker: discord followup non-2xx", { status: res.status });
    } catch (e) {
      console.warn("Worker: discord followup failed", { name: e?.name, msg: e?.message });
    }
  }
