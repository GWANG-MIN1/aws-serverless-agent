// Day 13 Worker handler — Agent Loop + executeCode 단일 도구.
//
// 입력 (event): { type: "run_chat", userId, sessionId, message, createdAtId }
//
// Day 12 와의 차이 (핵심): Bedrock 을 "한 번" 부르고 끝나던 것을 "루프" 로 키운다.
//
//   Day 12:  history → Converse 1회 → assistant Put → 끝.
//   Day 13:  history → [Converse → toolUse 있으면 코드 실행 → toolResult 되먹임] 반복
//                       → toolUse 없을 때(또는 MAX_TURN_STEPS) 종료.
//
// 원본 매핑 (packages/backend/src/agent-runtime/):
//   orchestrate.ts runChatTurn  → 아래 for(step) 루프
//   tools.ts       executeCode  → EXECUTE_CODE_TOOL (Bedrock Converse toolConfig 포맷으로 변환)
//   code-executor  node:vm      → runSandbox()
//   원본은 Anthropic SDK + TypeChecker + skills. 우리는 Bedrock Converse 로 옮기고
//   TypeChecker/skills 를 들어내 "간소화 sandbox" 로 줄였다 — Day 13 은 루프 흐름 한 가지.
//
